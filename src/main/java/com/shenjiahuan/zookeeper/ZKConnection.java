...
public class ZKConnection implements AsyncCallback.StatCallback {
  ...
  public ZKConnection(String host, GenericNode listener, String znode) {
    this.host = host;
  ...
  public void leaderElection() {
    try {
      createdNode =
          zoo.create(
              LEADER_ELECTION_ROOT_NODE + PROCESS_NODE_PREFIX,
              new byte[0],
  ...
  public void doLeaderElection() {
    ...
    try {
      childNodes = zoo.getChildren(LEADER_ELECTION_ROOT_NODE, false);
      final long createdIndex =
          Long.parseLong(
              createdNode.substring(
                  LEADER_ELECTION_ROOT_NODE.length() + PROCESS_NODE_PREFIX.length()));
